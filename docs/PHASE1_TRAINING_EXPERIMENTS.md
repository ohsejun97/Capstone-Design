...
| V9-A | 2026-04-01 | SaProt-650M + 3Di + ChemBERTa pooler_output (DAVIS) | 0.7889 | △ Morgan FP 미달 |
| V9-B | 2026-04-01 | SaProt-650M + 3Di + ChemBERTa pooler_output (KIBA) | 0.7602 | ❌ Morgan FP 대비 -0.043 |
| V10-A | 2026-04-01 | SaProt-650M + 3Di + ChemBERTa mean pooling (DAVIS) | 0.7915 | △ pooler보다 향상, 기준선 미달 |
| V10-B | 2026-04-01 | SaProt-650M + 3Di + ChemBERTa mean pooling (KIBA) | 0.7667 | △ pooler보다 향상, 기준선 미달 |
| **V11** | **2026-04-03** | **SaProt-650M + 3Di + ChemBERTa (BindingDB 32K약물)** | **0.8737** | **✅ 기준선 돌파 +0.0655** |
| V12 | 2026-04-03 | SaProt-650M + 3Di + GNN (BindingDB 32K약물) | 0.8411 | ✅ 기준선 돌파 +0.0329 |

---

## 최종 모델 선정

**채택: SaProt-650M FP16 + 3Di + ChemBERTa (BindingDB)**

| 기준 | 값 |
|------|-----|
| BindingDB Test r | **0.8737** (Phase 1f 최고) |
| BindingDB RMSE | 0.7933 |
| BindingDB CI | 0.8633 |
| Peak VRAM | 2.6GB (GTX 1650 SUPER ✅) |
| 학습 시간 | 141초 |
...
| 모델 | Test r | 데이터 | 비고 |
|------|--------|--------|------|
| SaProt+3Di + Morgan FP | 0.8082 | DAVIS | Phase 1c 기준선 |
| SaProt+3Di + ChemBERTa | **0.8737** | BindingDB | **Phase 1f 최종 채택** |
| SaProt+3Di + GNN | 0.8411 | BindingDB | 학습시간 7414초, 차선 |

**Agent Tool 1 (DTI 예측)**: Phase 1f 모델(ChemBERTa + BindingDB) 사용. 더 넓은 화학 공간(32K 약물 학습) → 신규 약물에 대한 일반화 성능 우수.
...
**Phase 1f 실험 결과 (2026-04-03):**

| 방식 | Dataset | Test Pearson r | RMSE | MAE | CI | Train Time | VRAM |
|------|---------|---------------|------|-----|-----|-----------|------|
| Morgan FP (기준) | DAVIS | 0.8082 | — | — | — | 56초 | ~2GB |
| ChemBERTa mean (frozen) | DAVIS | 0.7915 | 0.5627 | 0.3025 | 0.8608 | 61초 | 1.6GB |
| GNN (from-scratch) | DAVIS | 0.5795 | 0.7618 | 0.4032 | 0.7907 | 774초 | 2.1GB |
| **ChemBERTa mean (frozen)** | **BindingDB** | **0.8737** | **0.7933** | **0.5130** | **0.8633** | **141초** | **2.6GB** |
| GNN (from-scratch) | BindingDB | 0.8411 | 0.8842 | 0.5773 | 0.8459 | 7414초 | 2.2GB |

- Train / Val / Test: 56,556 / 8,079 / 16,160 (70/10/20 random split)
...
Phase 1f 완료 후 최종 DTI 모델을 Tool 1에 통합하고 smolagents ReAct 오케스트레이션 구현.
