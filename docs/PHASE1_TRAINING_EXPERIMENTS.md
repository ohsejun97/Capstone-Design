...
### Phase 1f — BindingDB 데이터 확장 + GNN/ChemBERTa 재시도 (🔄 In Progress)
...
| DAVIS | 68개 | r=0.58 ❌ |
| KIBA | 2,068개 | r=0.72 △ |
| BindingDB (예상) | 수만~수십만 개 | 미확인 |

KIBA에서는 DAVIS보다 훨씬 나은 결과(r=0.72)가 나왔다는 점이 방향을 시사한다 — **약물이 많을수록 GNN이 수렴한다.**

**전략:** BindingDB를 훈련 데이터로 사용하여 GNN 또는 ChemBERTa를 재시도.
...
- [x] 서버(500GB RAM)에서 전처리 완료 → `data/BindingDB/bindingdb_kd.csv` (60MB)
  - 필터링 후 115,426행 → 중복 dedup(평균 pKd) → **80,795 고유 쌍**
  - 고유 약물 32,480개 / 고유 타겟 2,384개 / pKd 2.00~13.33 (mean=6.25)
  - null 0건, 중복 쌍 0건 검증 완료
- [x] `train_dti_saprot.py` bindingdb/davis+bindingdb 케이스 CSV 읽기로 수정
- [ ] BindingDB 기반 GNN 또는 ChemBERTa 재훈련 및 평가
...
BindingDB_All.tsv (7.9GB, 305만 행)
        ...
        ↓ - 단일 체인 단백질만
        ↓ - Kd(nM) → pKd(-log10) 변환
        ↓ - 이상값 제거 (Kd > 10,000,000 nM)
data/BindingDB/bindingdb_kd.csv (smiles, sequence, pkd)
```
