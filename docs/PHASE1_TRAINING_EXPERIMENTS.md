...
| V9-A | 2026-04-01 | SaProt-650M + 3Di + ChemBERTa frozen (DAVIS) | 0.7889 | △ Morgan FP 미달 |
| V9-B | 2026-04-01 | SaProt-650M + 3Di + ChemBERTa frozen (KIBA) | 0.7602 | ❌ Morgan FP 대비 -0.043 |
...
|------|---------|--------|-------------|
| Morgan FP 단독 (기준) | **0.8082** | **0.8032** | — |
| ChemBERTa frozen (768-dim) | 0.7889 | 0.7602 | -0.019 / -0.043 |

**ChemBERTa 미달 원인 분석:**

1. **표현 차원 열세:** Morgan FP 2048-dim vs ChemBERTa 768-dim — MLP 헤드에 전달되는 정보량이 적음
2. **pooler_output 한계:** ChemBERTa의 pooler_output(CLS 토큰 후처리)는 분자 속성 예측 태스크용으로 최적화됨. DTI 친화도 예측에는 mean pooling 등 다른 aggregation이 더 적합할 수 있음
3. **소량 데이터에서의 Morgan FP 강점:** Morgan FP는 수십 년 화학 연구 기반의 결정론적 표현. DAVIS 68개처럼 작은 drug set에서도 안정적으로 작동

**현 단계 결론:** Drug encoder 개선이 성능 향상의 주요 경로가 아닐 수 있음. **Drug encoder는 Morgan FP로 확정**하고, Phase 3 Agent 오케스트레이션으로 이행.
