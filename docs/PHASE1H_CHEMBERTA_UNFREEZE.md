...
### 왜 ChemBERTa를 Unfreeze했는가?

#### 사전학습 목적과 DTI 태스크의 불일치

ChemBERTa는 PubChem 수백만 개 분자를 대상으로 **마스크 언어 모델(MLM)** 방식으로 사전학습됐다. 즉, SMILES 토큰 시퀀스에서 일부를 가리고 복원하는 방식으로 분자의 구조적 문법과 화학 성질을 학습한다.

| | ChemBERTa 사전학습 목적 | DTI pKd 예측 목적 |
|--|---|---|
| 태스크 | SMILES 토큰 복원 (MLM) | 약물-단백질 결합 친화도 수치 예측 |
| 필요한 표현 | 범용 분자 구조 표현 | 결합 친화도와 직접 연관된 화학 특성 |
| 관련 특징 | 원자 종류, 결합 구조, 관능기 | 소수성, 수소결합 수용체/공여체, 형태 보완성 |

Frozen 상태로는 이 간격을 메울 수 없다. Encoder는 자신의 원래 목적에 최적화된 표현을 고수한다.

#### 왜 전체가 아닌 상위 레이어만 Unfreeze했는가?

Transformer 언어 모델의 레이어는 계층적으로 역할이 분리된다:

- **하위 레이어 (0~3)**: 원자 유형, 결합 순서, 기본 관능기 같은 화학의 기본 문법을 인코딩. 어느 태스크에서도 공통적으로 유용하므로 frozen 유지.
- **상위 레이어 (4~5)**: 태스크 의존적 고수준 표현을 담당. DTI fine-tune 시 이 레이어가 pKd와 연관된 분자 특성(소수성 패치, 극성 표면적 등)을 강조하도록 적응.

전체 unfreeze는 과적합 위험이 크고(44M params, 56K 학습 쌍), 하위 레이어의 범용 표현을 손상시킬 수 있다.

#### 실험적 근거

| 단계 | ChemBERTa 상태 | 데이터셋 | 약물 수 | Pearson r |
|------|--------------|---------|--------|----------|
| 1e | frozen | DAVIS | 68 | 0.7915 ❌ |
| 1f | frozen | BindingDB | 32,480 | 0.8737 |
| **1h** | **layers 4~5 fine-tune** | **BindingDB** | **32,480** | **0.8923** |

1e 실패는 데이터 부족과 frozen 표현의 한계가 복합된 결과였다. 1f에서 데이터 문제를 해결한 뒤에도 frozen 표현이 r=0.8737에서 수렴했고, unfreeze를 통해 0.8923으로 개선됐다.
